...
inputs = {
    "message1": "Hello from container 1!",
    "message2": "Hello from container 2!"
}
...
def test_workflow():
            ...
            "templates": [
                {
                    ...
                    "container": {
                        ...
                        "command": ["sh", "-c", "echo '{{inputs.parameters.message1}}'; sleep 5"],
                        "resources": {
                            "limits": {
                                "cpu": "1"
                            }
                ...
                },
                {
                    ...
                    "container": {
                        "image": "my-container-image",
                        "command": ["sh", "-c", "echo '{{inputs.parameters.message2}}'; sleep 5"],
                        "resources": {
                            "limits": {
                                "cpu": "1"
                            }
    ...
    response = requests.post(api_url + "/namespaces/" + namespace + "/workflows", json=workflow)
...
while not workflow_completed:
    ...
    response = requests.get(api_url + "/namespaces/" + namespace + "/workflows/" + workflow_id)
...
response = requests.get(api_url + "/namespaces/" + namespace + "/workflows/" + workflow_id + "/outputs")
assert response.status_code == 200
