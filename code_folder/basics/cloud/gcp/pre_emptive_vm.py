...
def get_regular_vm_pricing(client, project, region, instance_type):
    sku_list = client.list_skus(
        project=project,
        service="compute.googleapis.com",
        parent=f"projects/{project}/services/compute.googleapis.com",
    )
    for sku in sku_list:
        if (
            sku.category.resource_group == "GCEInstance"
            and sku.category.usage_type == "OnDemand"
        ):
            if instance_type in sku.description:
                return (
                    float(
                        sku.pricing_info[0]
                        .pricing_expression.tiered_rates[0]
                        .unit_price.nanos
                    )
                    / 1e9
                )
...
while True:
    ...
    if regular_price is not None:
        ...
        print(
            f"Preemptible VM Price for {instance_type} in {region}: ${preemptible_price:.4f}"
        )
