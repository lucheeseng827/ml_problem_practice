...
def check_container_network(kube_client, namespace, container_name):
  ...
  if container_network.allow_external_connections:
    issues.append("The container is allowing external connections, which is insecure.")
  if container_network.allow_connections_from_pods_in_other_namespaces:
    issues.append("The container is allowing connections from pods in other namespaces, which is insecure.")

  return issues


def set_additional_network_latency(kube_client, namespace, pod_name, container_name, latency):
  """
  ...
  pod_network = kube_client.read_namespaced_pod_network_policy(
      namespace, pod_name)
  ...
  kube_client.update_namespaced_pod_network_policy(
      namespace, pod_name, pod_network)
=======
import logging
