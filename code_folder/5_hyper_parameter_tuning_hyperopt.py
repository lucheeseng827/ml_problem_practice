...
def build_model(input_size, hidden_size, num_classes, learning_rate):
    ...
    model.add(keras.layers.Dense(hidden_size, input_shape=(input_size,), activation='relu'))
...
def objective(params):
    if params['type'] == 'nn':
        model = build_model(input_size, params['hidden_size'], num_classes, params['learning_rate'])
    else:
